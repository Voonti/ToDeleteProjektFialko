...
void CProjectMFCView::OnDraw(CDC* pDC)
{
	...
	LOGFONT lf;
	font.GetLogFont(&lf);
	lf.lfHeight = -MulDiv(pDoc->fontSize, GetDeviceCaps(pDC->GetSafeHdc(), LOGPIXELSY), 72);
	CFont newFont;
	...
	for (int ipoint = 0; ipoint < npoints; ++ipoint)
	{
		...
		oldbrush = pDC->SelectObject(&newbrush);
		pDC->Ellipse(scr.x + pDoc->pointSize, scr.y + pDoc->pointSize, scr.x - pDoc->pointSize, scr.y - pDoc->pointSize);
		pDC->SelectObject(oldbrush);
		newbrush.DeleteObject();
		...
		//Output text
		pDC->SelectObject(&newFont);
		str.Format(_T("%s"), (*pDoc->pDat)[ipoint].name); // Format the string correctly
...
void CProjectMFCView::OnFileExport()
{
	CProjectMFCDoc* pDoc = GetDocument();
	if (pDoc && pDoc->pDat)
	{
		CFileDialog fileDlg(FALSE, _T("csv"), _T("export.csv"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("CSV Files (*.csv)|*.csv|All Files (*.*)|*.*||"));
		if (fileDlg.DoModal() == IDOK)
		{
			CString filePath = fileDlg.GetPathName();
			pDoc->pDat->exportToCSV(std::string(CT2CA(filePath)));
		}
	}
}
